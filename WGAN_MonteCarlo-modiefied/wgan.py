    ...
    def trainTTbar(self):
        ...
        for g_iter in range(self.generator_iters):
            ...
            for t in range(self.n_critic):
                ...
                if (images.size()[0] != self.batch_size & images_lat.size()[0] != self.batch_size): # the dataset may not be multiple of the batch size
                    continue
                real_data=self.get_torch_variable(images)
...
if __name__=="__main__":
    ...
    if ('train' in options.do_what) & ('ttbarGAN' not in options.model):
        model.trainIt()
        
    if ('train' in options.do_what) & ('ttbarGAN' in options.model):
        model.trainTTbar()
